...
/*
 * This is the class for parallel join executors, due to the fact that the
 * life cycle of right table and the way to build outer join results are quite
 * different from the sequential one.
 */
class AbstractParallelJoinExecutor : public AbstractExecutor {
  AbstractParallelJoinExecutor(const AbstractParallelJoinExecutor &) = delete;
  AbstractParallelJoinExecutor &operator=(
      const AbstractParallelJoinExecutor &) = delete;

 public:
  explicit AbstractParallelJoinExecutor(const planner::AbstractPlan *node,
                                        ExecutorContext *executor_context);

  virtual ~AbstractParallelJoinExecutor() {}
 ...
 protected:
  ...
  // TODO These three functions need adaption for parallel execution.
  ...
  // These tiles are wrapped by unique ptr because the executor has complete
  // ownership
  std::vector<std::unique_ptr<executor::LogicalTile>> left_result_tiles_;

  /** @brief Buffer to store right child's result tiles */
  // These tiles are raw ptr because right table tiles are owned by right
  // child
