...
//  const oid_t col_count = state.column_count + 1;
  ...
  numabench_database = new storage::Database(numabench_database_oid);
  catalog->AddDatabase(numabench_database);
...
//  }
  ...
  user_table = storage::TableFactory::GetDataTable(
      numabench_database_oid, user_table_oid, table_schema, table_name,
      DEFAULT_TUPLES_PER_TILEGROUP, own_schema, adapt_table);

  numabench_database->AddTable(user_table);
...
//  const oid_t col_count = state.column_count + 1;
  ...
  for (rowid = 0; rowid < tuple_count; rowid++) {
    ...
    for (oid_t col_itr = 1; col_itr < 3; col_itr++) {
