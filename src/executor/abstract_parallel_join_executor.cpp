...
//                         Peloton
//
// abstract_parallel_join_executor.cpp
//
// Identification: src/executor/abstract_parallel_join_executor.cpp
//
 ...
 */
AbstractParallelJoinExecutor::AbstractParallelJoinExecutor(
 ...
 */
bool AbstractParallelJoinExecutor::DInit() {
 ...
 */
std::vector<LogicalTile::ColumnInfo> AbstractParallelJoinExecutor::BuildSchema(
...
AbstractParallelJoinExecutor::BuildSchemaFromLeftTile(
  ...
  std::shared_ptr<storage::Tile> ptile(storage::TileFactory::GetTile(
      ...
      nullptr, *output_schema, nullptr, 1, LOCAL_NUMA_REGION));
...
std::vector<LogicalTile::ColumnInfo>
AbstractParallelJoinExecutor::BuildSchemaFromRightTile(
...
AbstractParallelJoinExecutor::BuildOutputLogicalTile(LogicalTile *left_tile,
...
std::unique_ptr<LogicalTile>
AbstractParallelJoinExecutor::BuildOutputLogicalTile(
...
std::vector<std::vector<oid_t>>
AbstractParallelJoinExecutor::BuildPostitionLists(LogicalTile *left_tile,
                                                  LogicalTile *right_tile) {
 ...
 */
void AbstractParallelJoinExecutor::BufferLeftTile(LogicalTile *left_tile) {
 ...
 * to the new result tile
 */
void AbstractParallelJoinExecutor::BufferRightTile(LogicalTile *right_tile) {
  PL_ASSERT(join_type_ != JOIN_TYPE_INVALID);
  PL_ASSERT(right_tile != nullptr);
 ...
 */
void AbstractParallelJoinExecutor::UpdateJoinRowSets() {
  PL_ASSERT(join_type_ != JOIN_TYPE_INVALID);
  switch (join_type_) {
    case JOIN_TYPE_LEFT:
      UpdateLeftJoinRowSets();
...
/**
  ...
  */
void AbstractParallelJoinExecutor::UpdateLeftJoinRowSets() {
...
/**
  ...
  */
void AbstractParallelJoinExecutor::UpdateRightJoinRowSets() {
...
/**
  ...
  */
void AbstractParallelJoinExecutor::UpdateFullJoinRowSets() {
  UpdateLeftJoinRowSets();
  UpdateRightJoinRowSets();
}
 ...
 */
bool AbstractParallelJoinExecutor::BuildOuterJoinOutput() {
  PL_ASSERT(join_type_ != JOIN_TYPE_INVALID);
...
bool AbstractParallelJoinExecutor::BuildLeftJoinOutput() {
  while (left_matching_idx < no_matching_left_row_sets_.size()) {
    ...
    if (right_result_tiles_.size() == 0) {
      ...
      auto right_tile = right_result_tiles_.front();
 ...
 * which doesn't have a match
 */
bool AbstractParallelJoinExecutor::BuildRightJoinOutput() {
  while (right_matching_idx < no_matching_right_row_sets_.size()) {
    ...
    std::unique_ptr<LogicalTile> output_tile(nullptr);
    auto right_tile = right_result_tiles_[right_matching_idx];
    LogicalTile::PositionListsBuilder pos_lists_builder;
